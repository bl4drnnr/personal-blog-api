/**
 * The content every environment starts from: one admin, the site copy, the CV,
 * and the handful of real posts. `seed` stops here; `seed:dev` builds bulk mock
 * content on top, which is why these are functions rather than a script body.
 */
...
/**
 * plainText and readingTimeMin are derived here exactly as the API derives them
 * on save, so seeded rows are searchable on the same terms as authored ones.
 */
export function insertPosts(db: SeedDb, seeds: PostSeed[]) {
...
export function seedSiteConfig(db: SeedDb) {
  return db.insert(schema.siteConfig).values({
    id: 1,
...
export async function seedAbout(db: SeedDb) {
...
/** The written-out posts, including the draft that draft handling is tested against. */
export function showcasePosts(): PostSeed[] {
  return [
    {
      type: 'article',
